...
public class GenerateCZITiles {
    public static void main(String[] args) throws FormatException, IOException {
        ...
        for (int i=0; i<args.length-1; i++) {
            switch (args[i]) {
                ...
                case "-u":
                   ...
                   / /System.load("/scratch/DMP/EVGENMOD/ntrahearn/lib/OpenCV/opencv/build/lib/libopencv_java401.so");
        ...
        if (para) {
            ...
            IntStream.range(start, end).parallel().forEach(i->{
                writeImagePara(imagePath, tP, i, ImageSize, res, blockSize, subBlockSize, nBlocks, rescale, sharpParams, bits);
            });
        } else { 
            for (int i=start; i<end; i++) {
                writeImageSerial(reader, tilePath, i, ImageSize, resolution, blockSize, subBlockSize, nBlocks, rescale, sharpParams, bits);
            }
    ...
    public static void writeImagePara(String imagePath, String folder, int index, int[] imageSize, int resolution, int[] blockSize, int[] subBlockSize, int[] nBlocks, double rescale, Double[] sharpParams, final int bits) {
        CZIReader reader;
        ...
        try {
            if (sharpParams[0] != null && sharpParams[1] != null && sharpParams[0] != 0 && sharpParams[1] != 0) {
                img = sharpen(img, sharpParams, index);
            }
    ...
    public static void writeImageSerial(CZIReader reader, String folder, int index, int[] imageSize, int resolution, int[] blockSize, int[] subBlockSize, int[] nBlocks, double rescale, Double[] sharpParams, final int bits) {
        double[] loadBlockSize = new double[]{(double)blockSize[0]/rescale, (double)blockSize[1]/rescale};
    ...
    private static synchronized BufferedImage sharpen(BufferedImage img, Double[] sharpParams, int i) {
        ...
        Mat sharpL = new Mat(byteImg.getHeight(), byteImg.getWidth(), CvType.CV_8U);
	...
	//Core.merge seems to throw assert exceptions in parallel mode
        ...
        BufferedImage sharpenedImage = new BufferedImage(byteImg.getWidth(), byteImg.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
        byte[] outData = ((DataBufferByte) sharpenedImage.getRaster().getDataBuffer()).getData();
