            ...
            if ~exist(fullfile(rms_dir(i).folder, rms_dir(i).name,'/img_box'),'dir')
                ...
                for i_sq = 1:length(sq_files)
                    ...
                    if ~endsWith(sq_files(i_sq).name,'T.png')
                        ...
                        %read the lumens
                        if exist(strrep(fullfile(sq_files(i_sq).folder, sq_files(i_sq).name),'box','lumen'),'file')
                        ...
                        %read the walls
                        if exist(strrep(fullfile(sq_files(i_sq).folder, sq_files(i_sq).name),'box','wall'),'file')
                        ...
                        if nnz(img_binary_sq)>.98*size(img_binary_sq,1)*size(img_binary_sq,2)
                            if size(img_raw,1)==size(img_raw,2)
                                ...
                                % calculated in uint8
                                img_binary_box_lumenANDwall = img_binary_box_lumen/max(img_binary_box_lumen(:)) + img_binary_box_wall./max(img_binary_box_wall(:));
                                img_binary_box_lumenOrwall = img_binary_box_lumenANDwall;
                                img_binary_box_lumenOrwall(img_binary_box_lumenOrwall>0) = 1;
                                ...
                                if display == 1
                                    ...
                                    subplot(235);imshow(img_binary_box_lumenANDwall);title('lumenANDwall')
                                    subplot(236);imshow(img_binary_box_lumenOrwall);title('lumenORwall')
                                    
                                    saveas(fig,['saving/',num2str(i0),'_',num2str(i),'_',num2str(i_sq),'.png'])
                                    close all
                                end
                                % save the results (extracted annotations)
                                if nnz(img_binary_box_lumenANDwall) > 0
...
%                                         mkdir(fullfile(savingpath,'img_box'));
                                        ...
                                        mkdir(fullfile(savingpath,'img_box_lumenANDwall'));
                                        mkdir(fullfile(savingpath,'img_box_lumenORwall'));
                                    end
%                                     imwrite(imgbox,strrep(fullfile(sq_files(i_sq).folder, sq_files(i_sq).name),'box','img_box'));
                                    ...
                                    imwrite(img_binary_box_lumenANDwall,strrep(fullfile(sq_files(i_sq).folder, sq_files(i_sq).name),'box','img_box_lumenANDwall'));
                                    imwrite(img_binary_box_lumenOrwall,strrep(fullfile(sq_files(i_sq).folder, sq_files(i_sq).name),'box','img_box_lumenORwall'));
                                end
                                clear img_raw img_binary_sq img_binary_lumen img_binary_wall img_binary_sq_lumen img_binary_sq_wall img_binary_box_lumenANDwall img_binary_box_lumenORwall
                            end
                        end
                    end
                end
